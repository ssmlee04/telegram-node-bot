...
class InputFile {
    ...
    prepareRequest(type, params) {
        return new Promise((resolve) => {
            ...
            if (Array.isArray(this._fileUrl)) {
              const multipart = { media: [] };
              this._fileUrl.map(({url, caption}) => {
                multipart['media'].push({
                    type: 'photo',
                    caption,
                    media: url
                });
              ...
              multipart['media'] = JSON.stringify(multipart['media'])
              resolve({ params: params, multipart: multipart})
            }
            if (!Array.isArray(this._fileUrl) && this._fileUrl) {
    ...
    static deserialize(raw) {
        ...
        if (Array.isArray(raw)) {
          return InputFile.isMediaGroup(raw);
        }
    ...
    static isMediaGroup(arr) {
        return new InputFile(null, null, arr.map(d => ({url: d.url, caption: d.caption})), null)
    }
}
